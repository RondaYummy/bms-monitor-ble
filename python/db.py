...
def fetch_all_data_range(from_dt: datetime, to_dt: datetime):
    try:
        with get_connection() as conn:
            ...
            cursor.execute('SELECT timestamp, current, power, device_address, device_name FROM bms_data WHERE timestamp BETWEEN ? AND ?', (from_str, to_str))
...
def fetch_all_data(days=None):
    ...
    try:
        with get_connection() as conn:
            ...
            cursor.execute('SELECT timestamp, current, power, device_address, device_name FROM bms_data WHERE timestamp >= ?', (cutoff_date_str,))
